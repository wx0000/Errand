...
**C1 — R1 — multi-location values rendered without separators.**
...
**C2 — R1 — no-salary offer still renders a salary element (placeholder + literal `null`).**
...
**C3 — R1 — salary rendered as raw JSON instead of a formatted range.**
...
**C4 — R3 — no-salary offer still renders a salary line in history.**
...
**C5 — R3 — salary rendered as raw JSON in history (same root as C3, history surface).**
...
> raw-JSON salary appears on both list (C3) and history (C5). Human decides if these are one
> defect or several.
