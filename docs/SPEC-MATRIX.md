...
What the suite must NOT assert. Each row is a way the tests could over-read the app: either rubber-stamping an adjudicated defect by asserting the broken output as correct, or asserting behaviour the spec never requires. The suite asserts the SPEC, not what the app happens to do.

| Anti-case (do NOT assert) | Why it over-interprets the app | Req |
|---|---|---|
| Concatenated locations (`WarsawGdansk`, `WarsawBerlinLondon`) treated as correct display | Spec requires a comma-separated list for multi-location offers; asserting the run-together string locks in BUG-A | R1 |
| A salary placeholder present for a no-salary offer (`null` on the list, `—` in history) | Spec forbids any placeholder when salary is absent, on the list and in history alike; asserting the placeholder rubber-stamps BUG-B | R1, R3 |
| Raw JSON salary (`{"min":10000,"max":15000}`) treated as the displayed value | Spec requires a formatted range like every other offer; asserting the JSON rubber-stamps BUG-C | R1, R3 |
| Snackbar asserted on the title alone (applied location omitted) | Spec requires title AND applied location in the confirmation snackbar; a title-only assertion would pass on an under-spec snackbar | R2 |
| Application history surviving an app restart | Spec is silent on restart; the app does persist (datastore, recon screens 12/13), but asserting persistence reads app behaviour, not a requirement | R3 |
