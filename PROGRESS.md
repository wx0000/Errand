...
- [ ] **P5 — Stabilization + delivery** — **repo-build slice DONE (this session); Desktop closeout
      (lead) pending.** Full P5 DONE = repo-build (Code) **+** Desktop closeout (lead).
  - **Repo-build (Claude Code) — DONE; local commits, not pushed:**
    - [x] Run the full suite 3× (stability) — **4 pass / 4 red, 0 flake, 0 state-flip** (lead ruled Gate 1)
    - [x] `BUGS.md` finalized — P2–P4 red-run artifacts linked (BUG-A→`01`, BUG-B→`04` + documented
          list-gap, BUG-C→`02`+`05`) alongside the recon evidence
    - [x] `TEST_PLAN` — `docs/TEST_PLAN.md` (R1–R5 matrix + anti-cases + ~2-min manual procedure)
    - [x] `README` — clean-clone bring-up + cardinal rule + architecture + design decisions + known
          limitations (incl. **BUG-B-on-list gap**) + "How this was built (AI-assisted)" + future work
    - [x] Fresh-clone verification — bring-up proof **4 pass / 4 red** (lead accepted as the pre-commit
          Gate-2 proof; clean-machine = human post-push)
    - [x] IP hygiene — **zero APK/PDF in tree + history**; `.idea/` git-ignored
  - **Lead/Desktop-owned — done in P5 OUTSIDE the repo-build (not this session):**
    - [ ] `docs/ai-history/WORKING-WITH-AI.md` consolidation — final English file generated by the lead
    - [ ] **`STRATEGIA-PROJEKT.md` section-10 sync** — two-way merge of the live project "instructions"
          field with section 10 (the file lives outside this repo). Do **NOT** overwrite blindly — each
          side holds content the other lacks; after merge master = "instructions", §10 = mirror, resync-dated.
...
- **Done:** P5 repo-build delivery — `README` (8 sections + "How this was built (AI-assisted)"),
  `docs/TEST_PLAN.md`, `docs/BUGS.md` red-run links, `.idea/` git-ignored, IP hygiene proven. Suite
  **3× stable** (4 pass / 4 red, 0 flake, 0 state-flip); **fresh-clone bring-up** 4 pass / 4 red. Five
  repo-build/closeout commits + one ai-history commit (plan archive + index), local only (**not
  pushed**). Plan archived → `docs/ai-history/p5-closeout.md`.
- **Decisions:** two lead-ruled stop-gates (stability, clean-machine); clean-machine deferred to the
  human post-push (Task F = the pre-commit bring-up proof); 3 meta-tasks = lead/Desktop-owned →
  **P5 not marked fully DONE**, repo-build slice only; README prereqs taken from the real env.
- **Drift:** none vs the approved (revised) plan. The original P5 closeout items (STRATEGIA §10 sync,
  orphaned-decision audit) plus WORKING-WITH-AI consolidation were reclassified by the lead as
  Desktop-owned and moved outside the repo-build; the `/export` transcript is the human's step.
